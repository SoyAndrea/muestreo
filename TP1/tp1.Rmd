---
title: "TP 1 - Diseño de muestra y estimadores"
...
# proporcion
Estproporcion <- survey::svyratio(~rancho_casilla,~Viv_radio,  design = diseno, 
...
#Seleccionaremos una muestra sistemática de los datos en radios_2010E4 
# utilizando el arranque aleatorio aa = 75
...
#Primero realizaremos la estimación del total poblacional usando el método de Horvitz-Thompson, 
# un estimador insesgado en muestreo sistemático.
...
# Arranque aleatorio
# aa <- sample(1:I, 1)
# La función arroja un aa de 191. Le colocamos # dado que sino arrojará un aa 
...
error_relRancho_CasillaE2 <- 100 * abs(ParametroRanchoCasillaE4 - estimRancho_CasillaE2) / 
                            ParametroRanchoCasillaE4
...
# Valor verdadero del parámetro poblacional 
parametro_CasaEst1 <- sum(Estrategia1E4$Casa)
...
# Definimos la función de estimación sistemática
estim_sistematico2r <- function(aa) {
  s = Estrategia2E4[seq(aa, N, I), ]
...
estimacionEst2 <- estim_sistematico2r(2)
...
lista_estimaciones2 <- lapply(lista_arranques2, estim_sistematico2r)
...
# Definimos la función de estimación sistemática
estim_sistematico2c <- function(aa) {
  s = Estrategia2E4[seq(aa, N, I), ]
  estim2 <- I * sum(s$Casa)
  return(c(estim2))
} 
...
estimacionEst2 <- estim_sistematico2c(2)
...
lista_estimaciones2 <- lapply(lista_arranques2, estim_sistematico2c)
...
# )
# 
# openxlsx::write.xlsx(datos4, "datos4.xlsx")
# openxlsx::write.xlsx(comparativa, "comparativa.xlsx")
...
- El sesgo es cero en ambas estrategias lo cual es un indicio de que ambas estrategias son estimadores insesgados del total de población, total de hogares casa y total de hogares rancho-casilla.

> La Estrategia 1, es decir el orden por Provincia y Total de viviendas, es la más optima ya que presenta menor coeficiente de variación(CV) y efecto de diseño(deff) en la tres unidades poblacionales calculadas, teniendo diferencias de casi 2 puntos con la estrategia 2 en tanto total de población y total de hogares que habitan vivienda tipo Casa, solo con el Total de hogares que habitan en viviendas rancho/casilla la diferencia entre estrategias es de 0.47-  
...
Cuando se calcula un intérvalo de confianza para la proporción utilizando el metodo de Clopper Pearson y en ninguna de las unidades de la muestra se observó cierta característica el intérvalo toma la forma: $$ \left(0 \ , \ 1 - \left(\frac{\alpha}{2}\right)^{\frac{1}{n}}\right) $$
